...
- ``id`` commands gives current user id, primary group, secondary group.

- ``id <username>`` gives ``username`` user id, primary, secondary group

```bash
id
# uid=1000(ram) gid=1000(ram) groups=1000(ram),4(adm),24(cdrom),27(sudo),30(dip),46(plugdev),100(users),114(lpadmin)
...
**Here:**
- uid => current user and user id.
- gid => current users primary group.
- groups => current users secondary group.
